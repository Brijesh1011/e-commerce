...
function PlaceOrder() {
    ...
    const { navigatehook, backendUrl, token, cartItems, setCartItems, delivery_fee, getCartAmount, products } = useContext(ShopContext);
            ...
            switch (Method) {
                ...
                case 'razorpay':
                    ...
                    if (responseRazorpay.data.success) {
                        console.log(responseRazorpay.data.order);  // Log Razorpay order details
                ...
                <div className='mt-12'>
                    ...
                    <div className='flex gap-3 flex-col lg:flex-row'>
                        <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                            <p className={`min-w-3.5 h-3.5 border rounded-b-full ${Method === 'stripe' ? 'bg-green-400' : ''}`}></p>
                            <img src={assets.stripe_logo} className='h-5 mx-4' alt="" />
                        </div>
                        
                        <div onClick={() => setMethod('cod')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                            <p className={`min-w-3.5 h-3.5 border rounded-b-full ${Method === 'cod' ? 'bg-green-400' : ''}`}></p>
